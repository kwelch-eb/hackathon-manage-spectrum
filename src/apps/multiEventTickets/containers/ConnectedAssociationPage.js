...
const _getActiveChildTicketIds = (state, {masterTicketId}) => (
    getActiveTicketAssociationsByMasterTicketId(state, masterTicketId).map((association) => association.childTicketId)
);
...
const _getFilteredByTicketIds = ({
    ...
    let filteredEventsWithTickets = Object.keys(selectedTicketsByEventId).reduce((eventList, eId) => {
        if (eId === eventId) {
            return eventList;
        }
...
const _getFilteredByEvents = ({
    ...
    Object.values(events).reduce((eventList, event) => {
        ...
        let eventTickets = allTicketIds
            .reduce((ticketList, ticketId) => {
                let ticket = ticketClasses[ticketId];
...
export const getFilteredEventsWithTickets = (state, {eventId, masterTicketId}) => {
    let {
        filters: {
            eventFilter = '',
            ticketFilter = '',
    ...
    if (selectedOnly) {
        let activeChildTicketIds = _getActiveChildTicketIds(state, {masterTicketId});
...
const _mapStateToProps = (state, {params: {eventId, masterTicketId}}) => {
    let initialAssociationStatuses = getAssociationAsChildStatus(state, {masterTicketId});
    let filteredEvents = getFilteredEventsWithTickets(state, {eventId, masterTicketId});
...
export default connect(_mapStateToProps, _mapDispatchToProps)(AssociationPage);
